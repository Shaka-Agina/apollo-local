...
export function FullPlayer({ onCollapse }: { onCollapse: () => void }) {
  ...
  return (
    // On mobile the player stops above the bottom tab bar so navigation stays
    // reachable; on desktop it covers the full viewport.
    <div className="fixed inset-x-0 top-0 z-[70] flex flex-col bg-base pb-6 pt-[calc(16px+env(safe-area-inset-top))] bottom-[calc(56px+env(safe-area-inset-bottom))] sm:bottom-0 sm:pb-[calc(24px+env(safe-area-inset-bottom))]">
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center px-8 py-6">
        <div className="flex aspect-square max-h-full w-full max-w-sm items-center justify-center overflow-hidden rounded-lg border border-edge bg-surface">
          {track.artwork ? (
